...
function resourceMetadataUrl(req, resourcePath) {
    ...
    try {
        switch (name) {
            case "list_available_services":
                ...
                return { content: [{ type: "text", text: JSON.stringify({ services: services.map((s) => ({ name: s.Title || s.TechnicalServiceName, technicalName: s.TechnicalServiceName, description: s.Description, path: s.ServiceUrl })) }, null, 2) }] };
